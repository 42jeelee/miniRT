...
/*   Updated: 2023/06/12 19:23:36 by ahkiler          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	cam->ray.dir = v_unit (v_sub_vec (v_add_vec (v_add_vec (cam->view_point_lb,
...
void	init_cam(t_data *data, t_ray ray, double ratio)
{
	...
	data->camera.horizontal = set_vec (data->camera.view_point_h, 0, 0);
	data->camera.vertical = set_vec (0, data->camera.view_point_h, 0);
	data->camera.view_point_lb = v_sub_vec (v_sub_vec (v_sub_vec (data->camera.ray.origin_point, focal)
